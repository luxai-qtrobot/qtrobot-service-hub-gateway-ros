...
class StreamBridge:
    ...
    def _setup(self, api_stream: dict, robot_ip: str) -> None:
        for api_path, info in api_stream.items():
            ...
            try:
                msg_class = get_msg_class(frame_type, self._interfaces_pkg)
    ...
    def _setup_out(self, api_path, endpoint, ros_topic, msg_class,
        ...
        interfaces_pkg = self._interfaces_pkg
        ...
        def reader():
            while self._running.is_set():
                try:
                    ...
                    # Unwrap magpie DictFrame envelope {gid, id, name, timestamp, value}
                    # when value is a dict (typed frames). For ListFrame (ImuFrame),
                    # keep the envelope so converters can extract value as a list.
    ...
    def close(self) -> None:
        self._running.clear()
        # Close subscribers/publishers first to unblock any pending sub.read() calls,
        # then join all threads (they exit immediately once the read unblocks).
