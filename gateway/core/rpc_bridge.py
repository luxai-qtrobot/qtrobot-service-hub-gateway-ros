...
class RpcBridge:
    """
    Creates ROS2 services for every entry in api_rpc and bridges each
    service call to the backend via ZMQ RPC.
    """
    ...
    def _setup(self, api_rpc: dict, robot_ip: str) -> None:
        for api_path, info in api_rpc.items():
            ...
            try:
                srv_class = get_srv_class(api_path, self._interfaces_pkg)
