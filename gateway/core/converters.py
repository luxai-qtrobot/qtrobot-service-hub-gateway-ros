"""
ROS2 ↔ ZMQ dict conversion helpers.
...
def get_srv_class(api_path: str, interfaces_pkg: str = 'qtrobot_interfaces'):
    """Return the <interfaces_pkg>.srv class for an RPC API path."""
    class_name = path_to_classname(api_path)
    mod = importlib.import_module(f'{interfaces_pkg}.srv')
    return getattr(mod, class_name)


def get_msg_class(frame_type: str, interfaces_pkg: str = 'qtrobot_interfaces'):
    """Return the <interfaces_pkg>.msg class for a frame type name."""
    mod = importlib.import_module(f'{interfaces_pkg}.msg')
...
def fill_ros2_rpc_response(zmq_resp, ros_response, returns_def: dict,
                           interfaces_pkg: str = 'qtrobot_interfaces') -> None:
    """
                ...
                if field_name in payload:
                    try:
                        ...
                        if isinstance(val, list):
                            val = [_coerce(v, field_type.rstrip('[]')) for v in val]
                        else:
                            val = _coerce(val, field_type)
                        setattr(item, field_name, val)
                    except Exception:
                        pass
        ros_response.result = item
...
def dict_to_ros2_msg(data_dict: dict, frame_type: str, frame_fields: dict,
                     sub_msgs_def: dict, ros2_msg, clock=None,
                     interfaces_pkg: str = 'qtrobot_interfaces') -> None:
    """
    ...
    Handles four cases:
      ...
      4. IMU frames (ImuFrame) — magpie ListFrame envelope with value: [x, y, z].
    """
    ...
    if frame_fields is None:
        # Built-in binary or special frame — use explicit field mapping
        if frame_type == 'AudioFrameRaw':
            ...
            ros2_msg.data = bytes(raw) if isinstance(raw, (bytes, bytearray)) else raw
        ...
        elif frame_type == 'ImuFrame':
            # Magpie ListFrame envelope: data_dict['value'] = [x, y, z]
            lst = data_dict.get('value')
            if isinstance(lst, list) and len(lst) >= 3:
                ros2_msg.x = float(lst[0])
                ros2_msg.y = float(lst[1])
                ros2_msg.z = float(lst[2])
...
def ros2_msg_to_dict(ros2_msg, frame_type: str, frame_fields: dict,
                     sub_msgs_def: dict,
                     interfaces_pkg: str = 'qtrobot_interfaces') -> dict:
    """
